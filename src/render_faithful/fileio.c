...
struct compr_rle_header {
	uint16_t srcsizel;
	uint8_t  srcsizeh;
	uint8_t  unk; // Always 0.
	uint8_t  esclen;
	uint8_t  esc[RS_VLE_ESC_LEN];
};
...
{
	uint16_t segm = FP_SEG(path);
	uint16_t offs = FP_OFF(path);
	FILE* handle;
...
{
	uint16_t segm = FP_SEG(dst);
	uint16_t offs = FP_OFF(dst);
...
{
	uint16_t segm = FP_SEG(src);
	uint16_t offs = FP_OFF(src);
...
int16_t fseek(FILE *file, int32_t offset, int16_t origin)
{
	uint16_t ol = offset;
	uint16_t oh = offset >> 16;
...
int16_t remove(const char* path)
{
	...
	__asm {
		...
		mov  dx, offs
		int16_t  21h
		jnc  int16_t unlink_ok
		mov  word ptr retval, -1
		mov  g_errno, ax
		jmp  int16_t unlink_done
	unlink_ok:
...
const char* file_find_next(void)
{
	...
	__asm {
		...
		jnc  int16_t findnext_ok
		mov  word ptr retval, -1
		jmp  int16_t findnext_done
	findnext_ok:
		mov  word ptr retval, 0
	findnext_done:
	}
...
uint16_t file_decomp_paras(const char* filename, int16_t fatal)
{
	...
	if ((file = fopen(filename, "rb")) != 0) {
		...
		if (!ferror(file)) {
			// May overflow, but all Stunts files are rather small.
			length = hdr.sizel | ((int32_t)hdr.sizeh << 16);
			return (length >> 4) + (length & 0xF ? 1 : 0);
		}
	}
...
uint32_t file_decomp_rle_seq(uint8_t huge* src, uint8_t huge* dst, uint32_t srclen, uint8_t esc)
{
	uint8_t cur, rep;
	uint8_t huge* seqstart, huge* seqend;

	uint8_t huge* srcend = src + srclen;
	uint8_t huge* dststart = dst;
...
uint32_t file_decomp_rle_single(uint8_t huge* src, uint8_t huge* dst, uint32_t len, uint8_t* esclookup)
{
	uint8_t cur, rep;
	uint16_t repw;

	uint8_t huge* dststart = dst;
	uint8_t huge* dstend = dst + len;
...
uint32_t file_decomp_rle(uint8_t huge* src, uint8_t huge* dst, uint16_t decompparas)
{
	uint32_t len, srclen, passlen;
	uint16_t skipseq, i;
	uint8_t esclookup[RS_RLE_ESCLOOKUP_LEN];
	uint8_t huge* origsrc;
	uint16_t paras;
...
uint32_t file_decomp_vle(uint8_t huge* src, uint8_t huge* dst, uint16_t decompparas)
{
	uint32_t len, lenleft;
	uint16_t additive, alphlen, width, widthdistr, i, j;
	uint16_t esc1[RS_VLE_ESC_LEN], esc2[RS_VLE_ESC_LEN];
	uint8_t alph[RS_VLE_ALPH_LEN], symb[RS_VLE_ALPH_LEN], wdth[RS_VLE_ALPH_LEN];
	uint8_t esclen, symbwdth, numsymb, numsymbleft, cursymb, tmp;
	uint8_t curwdt, nextwdt, code;
	uint16_t curword;
	...
	// Get decompressed size from header.
	hdr = (struct compr_header far*)src;
	len = lenleft = hdr->sizel | ((int32_t)hdr->sizeh << 16);
	src += sizeof(*hdr);
