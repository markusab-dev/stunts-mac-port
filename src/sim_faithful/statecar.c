...
extern uint8_t oppnentSped[10];

uint16_t update_rpm_from_speed(uint16_t currpm, uint16_t speed, uint16_t gearratio, int16_t changing_gear, uint16_t idle_rpm) {
	if (changing_gear == 0) {
		currpm = ((uint32_t)speed * gearratio) >> 16;
	}
...
/*
    ...
    jnz     int16_t loc_19DDB
    mul     [bp+arg_gearRatio]
    mov     cx, dx
loc_19DDB:
    cmp     cx, [bp+arg_idleRpm]
    jnb     int16_t loc_19DE3
    mov     cx, [bp+arg_idleRpm]
loc_19DE3:
    mov     ax, cx*/
}

void update_car_speed(char arg_carInputByte, int16_t arg_MplayerFlag, struct CARSTATE* arg_carState, struct SIMD* arg_simd) {
...
*/
	int16_t var_2;
	int16_t var_4;
	uint16_t var_updatedSpeed;
	int16_t var_deltaSpeed;
	uint8_t var_currTorque;

	/* Native port: the original build routed this to the 16-bit asm via
	 * ported_update_car_speed_(). The full C translation follows below,
	 * so the bypass is removed. */
...
/*
    ...
    jnz     int16_t loc_17A8E
    mov     [bp+var_2], 6
    jmp     int16_t loc_17A93*/
loc_17A8E:
	var_2 = 0xC;
loc_17A93:
...
/*    mov     si, [bp+arg_simd]
    ...
    jz      int16_t loc_17B86
    inc     [bx+CARSTATE.car_current_gear]
    jmp     int16_t loc_17B39*/
loc_17B20:
...
loc_17BDA:
	...
	goto loc_17C84;
/*    push    [bp+var_4]      ; int16_t
    call    _abs
    add     sp, 2
    cmp     ax, [bp+var_2]
    jg      int16_t loc_17BF6
    mov     bx, [bp+arg_carState]
    mov     si, bx
...
/*    mov     bx, [bp+arg_simd]
    ...
    add     sp, 2
    cmp     ax, [bp+var_2]
    jg      int16_t loc_17C40
    mov     bx, [bp+arg_carState]
    mov     si, bx
    mov     ax, [si+CARSTATE.car_knob_x2]
    mov     [bx+CARSTATE.car_knob_x], ax
    jmp     int16_t loc_17CAC*/
loc_17C40:
	if (var_4 <= 0)
		goto loc_17C52;
...
/*    mov     bx, [bp+arg_carState]
    ...
    cmp     [bx+CARSTATE.car_changing_gear], 0
    jz      int16_t loc_17DBC
    mov     [bx+CARSTATE.car_engineLimiterTimer], 0
    cmp     framespersec, 0Ah
    jnz     int16_t loc_17DB2
...
/*    mov     bx, [bp+arg_carState]
    cmp     [bx+CARSTATE.car_sumSurfRearWheels], 0
    jnz     int16_t loc_17DE6
    mov     si, [bp+arg_simd]
    mov     ax, [si+SIMD.max_rpm]
    cmp     [bx+CARSTATE.car_currpm], ax
    jb      int16_t loc_17DD4
    jmp     loc_17D39*/
loc_17DD4:
...
loc_17DE6:
	...
	goto loc_17E0C;
/*    cmp     [bx+CARSTATE.car_current_gear], 1
    jg      int16_t loc_17DFC
    cmp     [bx+CARSTATE.car_currpm], 0A28h
    jge     int16_t loc_17DFC
    mov     bx, [bp+arg_simd]
    mov     al, [bx+SIMD.idle_torque]
    jmp     int16_t loc_17E0C*/
loc_17DFC:
...
loc_17E0C:
	...
	if (arg_carState->car_currpm >= 0x1388)
		goto loc_17E34;
	var_currTorque = ((int16_t)arg_simd->idle_torque + var_currTorque) >> 1;
	/*
    mov     bx, [bp+arg_carState]
    cmp     [bx+CARSTATE.car_engineLimiterTimer], 0
    jz      int16_t loc_17E34
    cmp     [bx+CARSTATE.car_currpm], 1388h
    jge     int16_t loc_17E34
    mov     bx, [bp+arg_simd]
    mov     al, [bx+SIMD.idle_torque]
    sub     ah, ah
    ...
    mov     [bp+var_currTorque], al*/
loc_17E34:
	/* mov ax,[bx+car_gearratioshr8] ; mul cx ; shr ax,4 ; add var_deltaSpeed,ax
	 * `mul` is UNSIGNED and only AX survives the `shr`, so the product is
	 * truncated to 16 bits before shifting. */
	{
		uint16_t t = (uint16_t)((uint16_t)arg_carState->car_gearratioshr8 *
		                        (uint16_t)(uint8_t)var_currTorque);
		var_deltaSpeed = (int16_t)((uint16_t)var_deltaSpeed + (uint16_t)(t >> 4));
	}
	/* push mass as dword ; push 25 as dword ; push cwd(delta) ;
	 * call __aFlmul ; call __aFuldiv ; sar ax,1
	 *
	 * [ODDITY] __aFlmul is a SIGNED long multiply but __aFuldiv is UNSIGNED
	 * (the disassembly even remarks there is no clear reason to use it here).
	 * For a negative delta - which is the normal case at high speed, where
	 * drag exceeds gravity - the product is reinterpreted as a huge positive
	 * number, so the quotient is nothing like a signed divide would give.
	 * Only AX (the low word) reaches the final arithmetic shift.
	 * Reproducing this exactly is what makes the car lose speed through a
	 * gear change, as the original does. */
	{
		uint32_t prod = (uint32_t)((int32_t)var_deltaSpeed * 0x19L);
		uint32_t quot = prod / (uint32_t)(uint16_t)arg_simd->car_mass;
		var_deltaSpeed = (int16_t)((int16_t)(uint16_t)quot >> 1);
	}
...
/*    mov     al, [bp+var_currTorque]
	...
	if (arg_MplayerFlag == 0)
		goto loc_17EAD;
	var_currTorque = -((int16_t)*oppnentSped - 0xC8) >> 1;
	if (var_currTorque == 0)
		goto loc_17EAD;
	var_deltaSpeed -=  ((int32_t)var_currTorque * var_deltaSpeed) / 0xC8;
/*    cmp     [bp+arg_MplayerFlag], 0
    jz      int16_t loc_17EAD
    mov     al, oppnentSped
    sub     ah, ah
    sub     ax, 0C8h ; 'È'
    neg     ax
...
/*    mov     ax, [bp+var_deltaSpeed]
    add     [bp+var_updatedSpeed], ax
    cmp     [bp+var_updatedSpeed], 8000h
    jb      int16_t loc_17ED9
    cmp     [bp+var_updatedSpeed], 0F500h
    ja      int16_t loc_17ED9
    jmp     loc_17D5F*/
loc_17ED9:
...
loc_17EFB:
	if (var_4 <= 0x1400)
		goto loc_17F28;
	arg_carState->car_speed = ((int32_t)arg_carState->car_speed + arg_carState->car_speed2) >> 1;
	arg_carState->car_speed2 = arg_carState->car_speed;
	arg_carState->car_engineLimiterTimer = 5;
	goto loc_17F45;
/*    cmp     ax, 1400h       ; abs.delta = 20mph: a lot
    jle     int16_t loc_17F28
    mov     si, bx
