#!/usr/bin/env bash
# Build bin/intro_shot: the Phase 10 intro (rintro.c, rintro3d.c, rpes.c)
# with a standalone driver, so it can be run and screenshotted without
# src/main_native.c being involved at all.
#
# The object list is build_native.sh's, minus main_native.c and the audio
# front end, plus tools/intro_shot.c.
set -euo pipefail
...
clang -O2 -g \
  ...
  "${RF}"/rblit.c "${RF}"/rfileio.c "${RF}"/rstubs.c \
